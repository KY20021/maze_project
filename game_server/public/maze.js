...
let currentMoveState = document.getElementById("currentMoveState");
let currentPlayerPosition = document.getElementById("currentPlayerPosition");
...
function showRestartMessage() {
    ...
    messageBox.style.fontFamily = `'Lucida Sans', 
    'Lucida Sans Regular', 
    'Lucida Grande', 
    'Lucida Sans Unicode', 
...
/**
 * Moves the player based on the key pressed and updates the game state.
 *
 * @param {string} key - The key pressed by the user to move the player.
 * @param {Object} player - The player object containing x and y coordinates.
 * @param {number} player.x - The x-coordinate of the player.
 * @param {number} player.y - The y-coordinate of the player.
 */
function movePlayer(key, player) {
    ...
    switch (key) {
        ...
        case "ArrowLeft":
            if (
                player.x > 0 &&
                cells[player.x][player.y].walls.left === false
            ) {
                player.x--;
                points++;
                validMove = true;
    ...
    if (!validMove) {
        currentMoveState = document.getElementById("currentMoveState");
        currentMoveState.innerHTML = `invalid`;
        currentPlayerPosition = document.getElementById("currentPlayerPosition");
        currentPlayerPosition.innerHTML = `(${player.x}, ${player.y})`;
        return;
    }
    currentMoveState = document.getElementById("currentMoveState");
    currentMoveState.innerHTML = `valid`
    currentPlayerPosition = document.getElementById("currentPlayerPosition");
    currentPlayerPosition.innerHTML = `(${player.x}, ${player.y})`;
    ...
    if (player.x == cols - 1 && player.y == rows - 1) {
        ...
        messageBox.innerHTML += `<button id = 'done' onclick = 'location.reload()' >
        Play Again
        </button> `;
...
function drawPlayer(player) {
    ...
    pen.font = "12px Arial";
    pen.fillStyle = "black";
    pen.fillText(`(${player.x}, ${player.y})`, x - playerRadius, y - playerRadius - 5);
    pen.id = 'player'
}
